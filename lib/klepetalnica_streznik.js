...
var trenutniKanalGledeNaSocket= {};
...
function dodeliVzdevekGostu(socket, stGosta, vzdevkiGledeNaSocket, uporabljeniVzdevki) {
...
function pridruzitevKanalu(socket, kanal) {
  socket.join(kanal);
  trenutniKanalGledeNaSocket[socket.id] = kanal;
  ...
  socket.on('vzdevekSpremembaZahteva', function(vzdevek) {
    if (vzdevek.indexOf('Gost') == 0) {
      ...
      if (uporabljeniVzdevki.indexOf(vzdevek) == -1) {
        ...
        socket.broadcast.to(trenutniKanalGledeNaSocket[socket.id]).emit('sporocilo', {
...
function obdelajPridruzitevKanalu(socket) {
  socket.on('pridruzitevZahteva', function(kanal) {
    socket.leave(trenutniKanalGledeNaSocket[socket.id]);
